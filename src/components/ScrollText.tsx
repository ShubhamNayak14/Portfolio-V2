 ...
 * Top and bottom lines move in opposite directions to each other and opposite to the text.
 */
export default function ScrollText({
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (delta > 0) {
        // Scrolling down
        textX.set(textX.get() - speed); // text left
        topLineX.set(topLineX.get() + speed); // top line right
        bottomLineX.set(bottomLineX.get() - speed); // bottom line left
      } else {
        // Scrolling up
        textX.set(textX.get() + speed); // text right
        topLineX.set(topLineX.get() - speed); // top line left
        bottomLineX.set(bottomLineX.get() + speed); // bottom line right
      }
  ...
  return (
    ...
    >
      {/* Top moving line (right when scrolling down) */}
      <motion.div
      ...
      <motion.div
        style={{ x: smoothTextX }} className="flex whitespace-nowrap"
      >
        {[...Array(10)].map((_, i) => (
          <motion.h1
            key={i}
      ...
      {/* Bottom moving line (left when scrolling down) */}
      <motion.div
